...
jQdcc(document).ready(function($){
	...
	function init(){
		...
		$('.btn-watch-us').on('click', function(e){
			e.preventDefault();
            $('.wp-wrapper').addClass('slide');
            $('.watchus-popup').removeClass('deactive').addClass('active');
            setTimeout(function(){
		...
		$('.watchus-popup, .wp-btnclose').on('click', function(e){
			e.preventDefault();
            $('.wp-wrapper').removeClass('slide');
			$('.watchus-popup').removeClass('active').addClass('deactive');
            setTimeout(function(){
                $('.wp-container').html('');
            },150);
		});
...
function initLobby(){
	...
	$('.drag-nav-lobby .draggie.drag-next').on('mouseup mouseout touchend', function(e){
		e.preventDefault();
	...
	$('.drag-nav-lobby .draggie.drag-prev').on('mouseup mouseout touchend', function(e){
		e.preventDefault();
		clearInterval(interval);
		iv=1;
	});
...
function initRooftop(){
	...
	$('.drag-nav-rooftop .draggie.drag-next').on('mouseup mouseout touchend', function(e){
		e.preventDefault();
		clearInterval(interval);
		iv=1;
	});
	...
	$('.drag-nav-rooftop .draggie.drag-prev').on('mouseup mouseout touchend', function(e){
		e.preventDefault();
		clearInterval(interval);
		iv=1;
	});
...
function initOutspace(){
	...
	$('.drag-nav-outspace .draggie.drag-next').on('mouseup mouseout touchend', function(e){
		e.preventDefault();
		clearInterval(interval);
		iv=1;
	});
	...
	$('.drag-nav-outspace .draggie.drag-prev').on('mouseup mouseout touchend', function(e){
		e.preventDefault();
		clearInterval(interval);
		iv=1;
	});
	...
	var prevSlide = '';
	$('.drag-nav-outspace-mobile .draggie.drag-next').on('click', function(e){
		e.preventDefault();
		if(nextSlide != ''){
	...
	$('.drag-nav-outspace-mobile .draggie.drag-prev').on('click', function(e){
		e.preventDefault();
		if(prevSlide != ''){
...
function initPopup(){
	$('.dragon a').on('click', function(e){
		...
		if(!$(this).parent().hasClass('active')){
            ...
            $('.team-reveal-modal').addClass('slide-in');
		}else {
		...
		$('body').find('.team-reveal-modal a.close-reveal, .team-reveal-bg').on('click', function(e){
			e.preventDefault();
			$('.team-reveal-modal').removeClass('slide-in');
            setTimeout(function(){
...
function initReveal(){
	$('.dragon-hiring > a').on('click', function(e){
        ...
        $('.reveal-bg').css({"opacity": "1"});
        $('.reveal-modal').addClass('slide-in');
		disableNav(); 

		$('body').find('.reveal-modal a.close-reveal, .reveal-bg').on('click', function(e){
			e.stopPropagation();
			e.preventDefault();
			$('.reveal-bg').css({"opacity": "0"});
            $('.reveal-modal').removeClass('slide-in');
            setTimeout(function(){
